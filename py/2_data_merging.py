...
def merge_sheet(sheet_name):
    """读取并合并单个工作表"""
    try:
        df_a = pd.read_excel(_file_a, sheet_name=sheet_name)
        if sheet_name == '8、2017Q3后存续的813支转债标的':
            df_b = pd.read_excel(_file_b, sheet_name=sheet_name)
...
def run_data_merging(file_a, file_b, output_merged, output_dedup):
    """
    执行数据合并步骤
    ...
    Args:
        file_a (str): 历史数据文件路径
        file_b (str): 新数据文件路径
        output_merged (str): 合并后文件路径
        output_dedup (str): 去重后文件路径
    
    Returns:
        bool: True表示成功，False表示失败
    """
    global _file_a, _file_b
    ...
    try:
        ...
        if not merged_data:
            ...
            print(f"文件合并完成，已保存到 {output_merged}")

        # 2. 并行对合并后的文件按日期去重
        print(f"开始对 {output_merged} 进行去重，结果保存到 {output_dedup}")
        deduped_data = {}
        if not merged_data:
            print("警告：没有数据可供去重。")
        else:
        ...
        if not deduped_data:
            ...
            with pd.ExcelWriter(output_dedup, engine='xlsxwriter') as writer:
                for sheet_name, df in renamed_data.items():
                    df.to_excel(writer, sheet_name=sheet_name, index=False)
            print(f"去重完成，最终数据已保存到 {output_dedup}")
        
        print("数据合并步骤执行成功!")
        return True
        
    except Exception as e:
        print(f"数据合并过程中发生错误: {e}")
        return False
...
def main():
    ...
    output_merged = config.STEP2_OUTPUT_MERGED
    output_dedup = config.STEP2_OUTPUT_DEDUP
